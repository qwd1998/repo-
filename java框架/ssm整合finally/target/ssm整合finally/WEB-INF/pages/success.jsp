...
<%--${requestScope.roles}<br>--%>
<c:forEach items="${requestScope.roles}" var="role" varStatus="s" begin="0">
...
<%--${requestScope.users}--%>
<c:forEach items="${requestScope.users}" var="user" varStatus="s" begin="0">
    ${user}+${user.roles}+${user.accounts}+${s.index}<br>
</c:forEach>
<hr>
